...
def save_checkpoint(model: Hidden, experiment_name: str, epoch: int, checkpoint_folder: str):
    """Saves a checkpoint at the end of an epoch, keeping only last 5 checkpoints."""
    ...
    os.rename(tmp_path, checkpoint_path)  # Atomic replace
    
    # 3. Cleanup old checkpoints (keep last 5)
    try:
        # Get all matching checkpoints
        checkpoints = []
        for file in Path(checkpoint_folder).glob(f"{experiment_name}--epoch-*.pyt"):
            # Extract epoch number from filename
            match = re.search(r'epoch-(\d+)', file.name)
            if match:
                file_epoch = int(match.group(1))
                checkpoints.append((file_epoch, file))
        
        # Sort by epoch descending (newest first)
        checkpoints.sort(reverse=True, key=lambda x: x[0])
        
        # Delete older checkpoints beyond last 5
        for _, file in checkpoints[5:]:
            file.unlink()
            logging.info(f'Deleted old checkpoint: {file.name}')
            
    except Exception as e:
        logging.error(f'Error during checkpoint cleanup: {str(e)}')
